...
class MixtureModel:  # Works reliably for 2(+) Dimensional distributions
    ...
    def EMstacking(self):
        ...
        """[Xac dinh trong so hon hop cua tung model]"""
        iterations = 100
        for i in range(iterations):
            talpha = deepcopy(self.alpha)
            # Probability of every model (mixture weights) $\alpha$ in the main paper
    ...
    def mutate(self):
        ...
        # Determining std dev for mutation can be a parameteric study
        modifalpha = self.alpha + np.random.normal(0, 0.01,self.noms)
    ...
    def createtable(self, solutions, CV):
        if CV:
            self.noms = self.noms + 1  # NOTE: Last model in the list is the target model
            self.model_list.append(ProbabilityModel())
            self.model_list[self.noms - 1].buildmodel(solutions)
            self.alpha = (1 / self.noms) * np.ones(self.noms)
            nos = solutions.shape[0]    # so luong ca the trong quan the
            self.probtable = np.ones((nos, self.noms))
            # Calculating of the assigned probability of each source model to target solutions
